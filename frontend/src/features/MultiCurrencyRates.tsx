  ...
  return (
    <section className="glass-panel flex flex-col gap-4 p-6">
      <header className="flex items-center justify-between">
        ...
        <span className="flex items-center gap-2 rounded-full bg-brand-500/10 px-3 py-1 text-xs font-semibold text-brand-600">
      ...
      <ul className="flex flex-col gap-3 text-sm">
        ...
        {pairs.map((rate) => (
          <li key={`${rate.base}-${rate.date}`} className="flex items-center justify-between rounded-2xl bg-white/80 px-4 py-3 shadow-inner dark:bg-white/5">
            <div>
