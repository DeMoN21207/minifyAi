...
5. **Засейте тестовые данные.**

   ```bash
   npm run prisma:seed
   ```

   Команда создаст две тестовые учётные записи (администратора и обычного пользователя)
   с предустановленными паролями.
...
После запуска сидера доступны следующие логины и пароли для проверки ролей:

| Роль           | Email              | Пароль     |
|----------------|--------------------|------------|
| Администратор  | `admin@minify.ai`  | `Admin123!`|
| Пользователь   | `user@minify.ai`   | `User123!` |
